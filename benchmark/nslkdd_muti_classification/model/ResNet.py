...
class Model(FModule):
    def __init__(self, fc_hidden1=512, fc_hidden2=512, drop_p=0.5, num_classes=5):
    ...
    def forward(self, x_3d):
        ...
        x_3d = x_3d.view((x_3d.shape[0], 12, 12))  # torch.Size([512, 1, 12, 12])
        x = x_3d.unsqueeze(1)

        output = self.conv1_1(x) # torch.Size([512, 32, 12, 12])
        # x_1 = self.conv1_1(self.W * x_3d + x_3d)
        output = self.conv1_2(output) # torch.Size([512, 32, 12, 12])

        output = self.resnet(output) # torch.Size([512, 64, 12, 12])


        x = output.view(x.size(0), -1)  # flatten output of conv
