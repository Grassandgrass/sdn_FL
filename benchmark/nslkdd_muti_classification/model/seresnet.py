...
class Model(FModule):
    def __init__(self, block = BasicResidualSEBlock, num_block=[2,2,2,2], class_num=5):
    # def __init__(self, block=BottleneckResidualSEBlock, num_block=[3, 4, 6, 3], class_num=5):
        super().__init__()

        self.in_channels = 64

        # self.fc1 = nn.Linear(122, 144)
        #
    ...
    def forward(self, x):  #torch.Size([512, 196])
        ...
        output = self.pre(x) # 64*32*32  torch.Size([512, 64, 12, 12])
        print(output.shape)
        output = self.stage1(output)  # 64*32*32 torch.Size([512, 64, 12, 12])

        print(output.shape)
        output = self.stage2(output) # 128*16*16 torch.Size([512, 128, 6, 6])

        print(output.shape)
        output = self.stage3(output)  # 256*8*8 torch.Size([512, 128, 3, 3])
        print(output.shape)
        output = self.stage4(output)  # 512*4*4  torch.Size([512, 128, 2, 2])
