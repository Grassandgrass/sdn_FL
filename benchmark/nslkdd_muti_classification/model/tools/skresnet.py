...
class Model(FModule):
    ...
    def __init__(self, nums_class=5, block=SKBlock, nums_block_list=[3, 4, 6, 3]):
        ...
        self.conv=nn.Sequential(nn.Conv2d(1,64, 3, 1, 1,bias=False),
                                nn.BatchNorm2d(64),
                                nn.ReLU(inplace=True))
        self.maxpool=nn.MaxPool2d(3,2,1) # kernel=3x3 stride=2 padding=1
        self.layer1=self._make_layer(block,64,nums_block_list[0],stride=1) # 构建表中 每个[] 的部分
        ...
        self.fc=nn.Linear(512*block.expansion,nums_class) # 通道 2048 -> 1000
...
def SKNet50(nums_class=5):
    return Model(nums_class, SKBlock, [3, 4, 6, 3]) # 论文通过[3, 4, 6, 3]搭配出SKNet50
def SKNet101(nums_class=1000):
    return Model(nums_class,SKBlock,[3, 4, 23, 3])
if __name__=='__main__':
    x = torch.rand(100, 1, 12, 12)
    model=SKNet50()
    y=model(x)
