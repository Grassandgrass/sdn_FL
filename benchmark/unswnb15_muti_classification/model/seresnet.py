...
class Model(FModule):
    ...
    # def __init__(self, block=BottleneckResidualSEBlock, num_block=[3, 4, 6, 3], class_num=2):
        ...
        self.rnn = nn.LSTM(input_size=512, hidden_size=128, num_layers=1, batch_first=True, bidirectional=False)
    ...
    def forward(self, x):  #torch.Size([512, 196])
        ...
        output = self.pre(output) # 64*32*32  64*64*64
        ...
        output = self.dropout(output)  #torch.Size([8, 512])
        # output = self.rnn(output, 128)
        # output = rnn_h[-1, :, :]  # rnn_h => [b, 40]
