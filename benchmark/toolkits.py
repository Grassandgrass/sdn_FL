...
import benchmark.polyloss as polyloss
...
class ClassificationCalculator(BasicTaskCalculator):
    def __init__(self, device):
        ...
        self.criterion = torch.nn.CrossEntropyLoss(
            ...
            weight = torch.from_numpy(np.array([0.1, 0.6, 0.6, 2.0, 1.2])).float().cuda(),
            size_average = True,
        )
    ...
    def test(self, model, dataset, batch_size=64, num_workers=0):
        ...
        """
        # model = torch.load('D:/Project_Test/easyFL/benchmark/modelSave/test.pkl') # 加载模型
        model.eval()
        ...
        for batch_id, batch_data in enumerate(data_loader):
            ...
            y_predict = y_predict + y_pred.cpu().numpy().tolist()
            y_true = y_true + batch_data[-1].data.view_as(y_pred).cpu().numpy().tolist()
            # y_predict = y_predict + y_pred.numpy().tolist()
            # y_true = y_true + batch_data[-1].data.view_as(y_pred).numpy().tolist()  # 没有GPU时返回的pred和true

        torch.save(model,'D:/Project_Test/easyFL/benchmark/modelSave/test.pkl')
        torch.save(model.load_state_dict, 'D:/Project_Test/easyFL/benchmark/modelSave/test.pth')
        # torch.save(model,'D:/Project_Test/easyFL/benchmark/modelSave/nslkdd_muti_classification_cnum10_FLResNet18.pkl')
        # python main.py --task nslkdd_muti_classification_cnum10_dist0_skew0_seed0 --model 3LMNet --algorithm fedavg --num_rounds 30 --num_epochs 5 --learning_rate 0.0100 --proportion 0.215 --batch_size 10 --eval_interval 1
        # 第22伦达到最优值acc = 0.84, 模型参数nslkdd_muti_classification_cnum10_FLResNet18.pkl
        return {'accuracy': 1.0 * num_correct / len(dataset), 'loss': total_loss / len(dataset), 'y_predict': y_predict, 'y_true': y_true}  # 计算本轮的acc和loss ,及每个类的acc
